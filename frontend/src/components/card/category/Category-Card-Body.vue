...
export default {
  ...
  computed: {
    filteredProducts() {
      if (this.selectedCategory) {
        return this.cardData.filter(product => product.category === this.selectedCategory);
      } else {
  ...
  mounted() {
    axios
        .get(`http://localhost:8081/api/products?category=${this.selectedCategory}`, { withCredentials: true })
        .then(response => {
